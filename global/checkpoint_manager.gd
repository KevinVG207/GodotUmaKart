...
class CheckpointSegment:
	...
	var checkpoints: Array[Checkpoint2] = []
	var first: Checkpoint2:
		get:
			return checkpoints[0]
	var last: Checkpoint2:
		get:
...
func initialize_checkpoints(checkpoints: Array[Checkpoint2]) -> void:
...
func create_segments(checkpoints: Array[Checkpoint2]) -> Array[CheckpointSegment]:
	var checkpoints_left: Array[Checkpoint2] = checkpoints.duplicate()
	var cur_segment: CheckpointSegment = CheckpointSegment.new()
	segments = []
	var cur_checkpoint: Checkpoint2 = null
	...
	while checkpoints_left.size() > 0:
		...
		if cur_checkpoint.next_points.size() != 1 or cur_checkpoint.next_points[0].prev_points.size() != 1:
			segments.append(cur_segment)
			cur_segment = CheckpointSegment.new()
			cur_checkpoint = null
		else:
			cur_checkpoint = cur_checkpoint.next_points[0] as Checkpoint2
