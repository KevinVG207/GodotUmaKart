...
func do_engine_sound() -> void:
	...
	var engine_volume: float = make_volume(remap(abs(parent.cur_speed), 3, 10, 0.75, 1))
...
func do_drift_sound() -> void:
	...
	# parent is drifting
	if skid_stream == -1:
		skid_stream = engine_playback.play_stream(skid_sound, 0, make_volume(0.2), 1.0, AudioServer.PLAYBACK_TYPE_STREAM, "SFX")
...
func water_entered() -> void:
	play_sample(water_splash_sound, 0.2, randf_range(1.15, 1.25))
